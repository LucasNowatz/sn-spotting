"""Forward chemistry-transport model used to build the Problem 3 ozone archive.
...
* Gas-phase chemistry: KEPLER-RED v1.2, a 12-species reduced NOx-VOC mechanism
...
# --------------------------------------------------------------------------
# KEPLER-RED v1.2 gas-phase chemistry
# --------------------------------------------------------------------------
...
def write_grid(path):
    with Dataset(path, "w", format="NETCDF4") as ds:
        ...
        ds.title = "Problem 3 ozone budget archive: grid and topology"
        ds.dx_metres = DX
        ds.dy_metres = DY
        ds.face_orientation = (
...
def write_meteo(path, air_mass, spec_hum, times):
    with Dataset(path, "w", format="NETCDF4") as ds:
        ...
        ds.title = "Problem 3 ozone budget archive: meteorology"
...
def write_masks(path, fixed, moving, times):
    with Dataset(path, "w", format="NETCDF4") as ds:
        ...
        ds.title = "Problem 3 ozone budget archive: diagnostic volumes"
...
def write_state(path, o3_ppbv, times):
    with Dataset(path, "w", format="NETCDF4") as ds:
        ...
        ds.title = "Problem 3 ozone budget archive: ozone state"
...
def write_process(path, proc, t0, t1):
    with Dataset(path, "w", format="NETCDF4") as ds:
        ...
        ds.title = "Problem 3 ozone budget archive: integrated process increments"
        ds.mechanism = "KEPLER-RED v1.2"
        ds.completeness = (
            "the six gross channels o3_prod_jno2 and o3_loss_* are every reaction "
            "in KEPLER-RED v1.2 that creates or destroys O3; o3_prod_ox_diag is a "
...
def write_fluxes(path, fl, t0, t1):
    with Dataset(path, "w", format="NETCDF4") as ds:
        ...
        ds.title = "Problem 3 ozone budget archive: integrated interface fluxes"
        ds.independence = (
...
def write_manifest(runs):
    ...
    man = {
        "archive_version": "kepler-o3-budget-archive-1.0",
        "mechanism": "KEPLER-RED v1.2",
